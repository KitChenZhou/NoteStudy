...
/**
 * Created by D22395 on 2017/8/29.
 * 创建表格和写入表格
 */
...
class SaveToExcelUtil {
    ...
    //将数据存入到Excel表中
    static void writeToExcel(File excelFile, ArrayList<CallRecord> callRecordList) {
        ...
        try {
            ...
            for (int i = 0; i < callRecordList.size(); i++) {
                CallRecord callRecord = callRecordList.get(i);

                WritableCellFormat wcf;
                if (callRecord.isConnect().equals("pass")) {
                    wcf = getAlignment();
                } else {
                    wcf = getBackground();
                }
                lab1 = new Label(0, i + 1, callRecord.getId() + "", wcf);
                lab2 = new Label(1, i + 1, callRecord.getStartDate(), wcf);
                lab3 = new Label(2, i + 1, callRecord.getEndDate(), wcf);
                lab4 = new Label(3, i + 1, callRecord.getDuration() + "", wcf);
                lab5 = new Label(4, i + 1, callRecord.getNetworkType(), wcf);
                lab6 = new Label(5, i + 1, callRecord.isConnect(), wcf);
                ws.addCell(lab1);
                ws.addCell(lab2);
                ws.addCell(lab3);
                ws.addCell(lab4);
