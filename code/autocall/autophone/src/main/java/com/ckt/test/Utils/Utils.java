...
public class Utils {
        ...
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            ...
            dialog.setPositiveButton("继续", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialogInterface, int i) {
                    ...
                    SaveToExcelUtil.writeToExcel(file, callLogList);
     ...
     * 获取报告
     * @param context 上下文
     * @param callLogList 存储callLog的数组
     */
...
//                    Utils.showToast(MainActivity.this, "有效通话" + mValidCall + "次，" + "其中详细信息：" + callLogList.toString());
        AlertDialog.Builder dialog = new AlertDialog.Builder(context);
        dialog.setTitle("通话信息");
        dialog.setMessage("成功拨打次数： " + validCall + "\n"
                + "失败拨打次数： " + (callLogList.size() - validCall) + "\n"
                + "总计拨打次数： " + callLogList.size() + "\n"
                + "      呼通率      ： " + (validCall * 1.0) / callLogList.size() * 100 + "%");
        dialog.setCancelable(true);
    ...
    public static void CheckSimCardState(Context context) {
        ...
        if (imsi != null) {
            if (imsi.startsWith("46000") || imsi.startsWith("46002")) {
                Utils.showToast("中国移动:" + Utils.getNetworkClass(tm.getNetworkType()));
            } else if (imsi.startsWith("46001")) {
                Utils.showToast("中国联通:" + Utils.getNetworkClass(tm.getNetworkType()));
            } else if (imsi.startsWith("46003")) {
                //中国电信
                Utils.showToast("中国电信:" + Utils.getNetworkClass(tm.getNetworkType()));
            } else {
                Utils.showToast("SIM不可用");
            }
        } else {
            Utils.showToast("SIM不可用");
        }
