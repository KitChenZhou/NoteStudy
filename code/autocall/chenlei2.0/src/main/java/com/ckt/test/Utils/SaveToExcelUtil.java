...
class SaveToExcelUtil {
    ...
    //将数据存入到Excel表中
    static void writeToExcel(File excelFile, CallRecord callLogUtil) {
        ...
        try {
            ...
            if (callLogUtil.isConnect().equals("pass")) {
                wcf = getAlignment();
            } else {
                wcf = getBackground();
            }
            lab1 = new Label(0, row, callLogUtil.getId() + "", wcf);
            lab2 = new Label(1, row, callLogUtil.getStartDate() + "", wcf);
            lab3 = new Label(2, row, callLogUtil.getEndDate() + "", wcf);
            lab4 = new Label(3, row, callLogUtil.getDuration() + "", wcf);
            lab5 = new Label(4, row, callLogUtil.getNetworkType() + "", wcf);
            lab6 = new Label(5, row, callLogUtil.isConnect() + "", wcf);
            ws.addCell(lab1);
            ws.addCell(lab2);
            ws.addCell(lab3);
            ws.addCell(lab4);
